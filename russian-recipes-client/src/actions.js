...
// THUNK - is a function that that wraps an expression to delay its evaluation, it allows us to handle async better. THUNK is grabbing our argument from DISPATCH, intercepting the communication between DISPATCH and REDUCER, and then THUNK is checking the argument: if it's an object it doesn't do anything and lets it pass to the REDUCER, but if it's a fucntion - it invokes that fucntion (runs that f) and inside of pur fucnction we will be returning an object and then it will take that object and continue with that communication
